...
import { generateJson, llmConfigured, llmErrorMessage, llmErrorStatus } from "../adapters/llm";
import { loadSession } from "../adapters/store";
import { getDemoSummaries } from "../demo-transcript";
...
export async function handleSummarize(request: Request, env: AppEnv): Promise<Response> {
  ...
  if (ctx.demo || !llmConfigured(env)) {
    const summaries = getDemoSummaries(ctx.videoId) ?? {};
    const seed = summaries[section.title];
    const exampleTitle = Object.keys(summaries)[0];
    logger.debug("summarize 演示模式分支", { sessionId, sectionTitle, hit: !!seed });
    if (seed) return jsonResponse({ summary: seed, demo: true });
    return jsonResponse(
      {
        error: exampleTitle
          ? `演示模式仅「${exampleTitle}」章节内置 5W1H 示例；配置 GEMINI_API_KEY 或 DEEPSEEK_API_KEY 后即可总结任意章节`
          : "此演示视频无内置 5W1H 示例；配置 GEMINI_API_KEY 或 DEEPSEEK_API_KEY 后即可总结任意章节",
      },
      503,
    );
  }
